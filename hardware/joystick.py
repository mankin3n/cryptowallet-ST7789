...
Hardware: 5 tactile push buttons
    ...
    def start(self) -> None:
        ...
        if _BUTTON_MOCK_MODE:
            ...
            try:
                ...
                GPIO.setup(config.BUTTON_RIGHT_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                ...
                logger.info(f"  - RIGHT: GPIO {config.BUTTON_RIGHT_PIN}")
    ...
    def _poll_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                # Check RIGHT button
                if self._read_button(config.BUTTON_RIGHT_PIN):
                    if (current_time - self.last_button_times[config.BUTTON_RIGHT_PIN]) > (config.BUTTON_DEBOUNCE_MS / 1000.0):
                        self.event_queue.put(JoystickEvent(config.INPUT_RIGHT))
                        self.last_button_times[config.BUTTON_RIGHT_PIN] = current_time
